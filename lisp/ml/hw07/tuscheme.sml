...
(* types for {\tuscheme} ((answers)) S652c *)
fun pairtype (a, b) = CONAPP (TYCON "pair",  [a, b])
fun sumtype  (a, b) = CONAPP (TYCON "sum",   [a, b])
fun qtype    a      = CONAPP (TYCON "queue", [a])
fun arraytype a     = CONAPP (TYCON "array", [a])
fun reftype a       = CONAPP (TYCON "ref", [a])
val tyvarB = TYVAR "'b"
val tyvarC = TYVAR "'c"
...
fun tysubst (tau, varenv) =
  ...
  (* definition of [[renameForallAvoiding]] for {\tuscheme} ((prototype)) 397 *)
      fun renameForallAvoiding (alphas, tau, captured) =
        let val tauFrees = freetyvars tau
            fun renameAndAdd (alpha, (alpha's, bad)) =
              let val alpha' = if not (member alpha bad) then alpha
                               else freshName (alpha, union (tauFrees, bad))
              in  (alpha' :: alpha's, insert (alpha', bad))
              end
            val (alpha's, _) = foldr renameAndAdd ([], captured) alphas
            val renameEnv = mkEnv (alphas, map TYVAR alpha's)
        in  FORALL (alpha's, tysubst (tau, renameEnv))
        end
      (* type declarations for consistency checking *)
...
(* type declarations for consistency checking *)
val _ = op instantiate : tyex * tyex list -> tyex
...
fun kindPass (e, delta) = 
  let
      fun ki (LITERAL v) = true
        | ki (VAR name)  = true
        | ki (SET (name, e)) = ki e
        | ki (IFX (e1, e2, e3)) = (ki e1) andalso (ki e2) andalso (ki e3)
        | ki (WHILEX (e1, e2)) = (ki e1) andalso (ki e2)
        | ki (BEGIN es) = List.all ki es
        | ki (APPLY (e, elist)) = List.all ki (e::elist)
        | ki (LETX (LET, bs, e)) = 
            kindPass (e, delta) andalso
            let val (names, es) = ListPair.unzip bs
            in  List.all ki es
            end 
        | ki (LETX (LETSTAR, b::bs, e)) =
            ki (LETX (LET, [b], LETX (LETSTAR, bs, e)))
        | ki (LETX (LETSTAR, [], e)) = ki e
        | ki (LETRECX (typedBs, e)) =
            let val (nameTypePairs, es) = ListPair.unzip typedBs
                val (names, types)      = ListPair.unzip nameTypePairs
                val _                   = List.map (fn x => asType (x, delta))
                                                   types
            in List.all ki (e::es)   
            end 
        | ki (LAMBDA (bs, e)) =  
            let val (names, ts) = ListPair.unzip bs
                val _           = List.map (fn x => asType (x, delta)) ts
            in ki e end
        | ki (TYLAMBDA (names, e)) = 
            let val kinds    = List.map (fn _ => TYPE) names
                val newDelta = bindList (names, kinds, delta)
            in kindPass (e, newDelta)
            end  
        | ki (TYAPPLY (e, ts)) =
            let val _   = List.map (fn x => asType (x, delta)) ts
            in  ki e
            end
  in ki e
  end
...
fun kinddef (d, delta) = 
  let 
    fun kd (VAL (name, e)) = kindPass (e, delta)
      | kd (VALREC (name, tau, (LAMBDA l))) = 
          let val _        = asType (tau, delta)
          in  kindPass (LAMBDA l, delta)
          end
      | kd (VALREC (name, tau, _)) = 
          raise TypeError "Letrec must define lambdas"
      | kd (EXP e) = kd (VAL ("it", e))
      | kd (DEFINE (name, tau, (bs, e))) =
          let val (names, types) = ListPair.unzip bs
          in  kd (VALREC (name, FUNTY (types, tau), LAMBDA (bs, e)))
          end
  in kd d
  end

fun typeofwrapper (e, delta, gamma) = 
  let val _ = kindPass (e, delta)
  in typeof (e, gamma)
  end

fun typdefwrapper (e, delta, gamma) = 
  let val _ = kinddef(e, delta)
  in typdef (e, gamma)
  end
...
fun processDef (d, (Delta, Gamma, rho), interactivity) =
  let val (Gamma, tystring)  = typdefwrapper (d, Delta, Gamma)
...
fun testIsGood (test, (Delta, Gamma, rho)) =
  let fun ty e = typeofwrapper (e, Delta, Gamma)
                 handle NotFound x =>
...
(* shared [[checkAssertPasses]] and [[checkErrorPasses]], which call [[outcome]] S224c *)
      ...
      fun deftystring d =
        snd (typdefwrapper (d, Delta, Gamma))
        handle NotFound x =>
          raise TypeError ("name " ^ x ^ " is not defined")
