...
(* freeIn : exp -> name -> bool
   freeIn e x returns true iff variable x occurs free in μScheme
   expression e. *)
fun freeIn e x =
  let
    fun free exp =
      case exp of
        ...
        | APPLY (f, args) =>
            free f orelse List.exists free args
        ...
        | IFX (e1, e2, e3) =>
            free e1 orelse free e2 orelse free e3

        | WHILEX (e1, e2) =>
            free e1 orelse free e2

        | LETX (LET, bindings, body) =>
            let
              val names = map fst bindings
              val exprs = map snd bindings
            in
              List.exists free exprs orelse
              (if member x names then false else free body)
            end
        ...
        | LETX (LETREC, bindings, body) =>
            ...
            in
              if member x names
              then List.exists free exprs
              else List.exists free exprs orelse free body
            end
...
(* Unit Test *)
        ...
        val () =
            Unit.checkAssert "freeIn set var false"
            (fn () => not (freeIn set_yx "y"))
        end
        ...
        val () =
        Unit.checkAssert "x free in lambda"
            (fn () => freeIn lambda_y "x")

        val () =
        Unit.checkAssert "y not free in lambda"
...
(*                                                               *)
(*****************************************************************)
(* Type: (name list * exp) * 'a env -> (name list * exp) * 'a env 
    Optimizes closures by "shrinking" the captured environment. 
    It uses freeIn to identify which variables in the environment are actually 
    needed by the function body and filters out the rest.  *)
fun improve ((formals, body), rho) =
  let
    fun isFree (y, _) =
      freeIn body y andalso not (member y formals)
  in
    ((formals, body), List.filter isFree rho)
  end

(*** Unit tests ***)

        val mock_env = [("x", NUM 1), ("y", NUM 2), ("z", NUM 3)]
        val mock_lambda = (["y"], APPLY (VAR "+", [VAR "x", VAR "y"]))


        val improved_closure = improve (mock_lambda, mock_env)

        val () =
            Unit.checkAssert "improve keeps x"
            (fn () => List.exists (fn (n, _) => n = "x") (snd improved_closure))

        val () =
            Unit.checkAssert "improve removes y (formal)"
            (fn () => not (List.exists (fn (n, _) => n = "y") (snd improved_closure)))

        val () =
            Unit.checkAssert "improve removes z (unused)"
            (fn () => not (List.exists (fn (n, _) => n = "z") (snd improved_closure)))
