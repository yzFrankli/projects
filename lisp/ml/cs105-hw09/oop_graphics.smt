...
(class Point
  ...
  ;; inbetween takes in a LineSegment seg, and tells whether the current point
  ;; lies on seg.
  (method inSeg: (seg) ;; private
    [locals x1 y1 x2 y2]
    (set x1 (seg x1))
    (set y1 (seg y1))
    (set x2 (seg x2))
    (set y2 (seg y2))
    (((((x1 - epsilon) <= x) & (x <= (x2 + epsilon))) | 
      (((x2 - epsilon) <= x) & (x <= (x1 + epsilon)))) 
     &
     ((((y1 - epsilon) <= y) & (y <= (y2 + epsilon))) | 
      (((y2 - epsilon) <= y) & (y <= (y1 + epsilon))))))
...
(class Intersect
  ...
  (method inline:With (env) (Intersect withE1:e2: (e1 inline:With env) (e2 inline:With env)))
...
;; Put your unit tests for preprocessing here!
        ...
        (check-assert (close:GeometryValue: value:value:
                          ((LineSegment withX1:y1:x2:y2: 
                                3.2 ~4.1 (3.2 + smidge) 4.1) preprocessProg)
